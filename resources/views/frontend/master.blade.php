...
<head>
    ...
    <title>Themart - @forelse (Request::segments() as $segment ){{ ucwords($segment) }} @empty Home @endforelse</title>
...
<body>
    ...
    <script type="text/javascript">
        ...
        s1.async=true;
        s1.src='https://embed.tawk.to/65bf0f5e8d261e1b5f5c18a4/1hlp582ed';
        s1.charset='UTF-8';
                    ...
                    <div class="row align-items-center">
                        ...
                        <div class="col-lg-6 col-12">
                            <div class="middle-box">
                                <div class="category">
                                    <select name="service" class="form-control single_category">
                                        ...
                                        <div class="header-wishlist-form-wrapper">
                                            ...
                                            <div class="mini-wislist-content">
                                                ...
                                                <div class="mini-cart-items">
                                                    @forelse ( App\Models\Wishlist::where('customer_id',Auth::guard('customer')->id())->get() as $wishlist )
                                                    <div class="mini-cart-item clearfix">
                                                        <div class="mini-cart-item-image">
                                                            <a href="{{ route('product.single',$wishlist->rel_to_product->slug) }}"><img src="{{ asset('uploads/product/preview') }}/{{ $wishlist->rel_to_product->preview }}" alt></a>
                                                        </div>
                                                        <div class="mini-cart-item-des">
                                                            @if (strlen($wishlist->rel_to_product->product_name)>20)
                                                                <a href="{{ route('product.single',$wishlist->rel_to_product->slug) }}" title="{{ $wishlist->rel_to_product->product_name }}">{{ Str::substr($wishlist->rel_to_product->product_name,0,20).'..' }}</a>
                                                            @else
                                                                <a href="{{ route('product.single',$wishlist->rel_to_product->slug) }}">{{ $wishlist->rel_to_product->product_name }}</a>
                                                            @endif
                                                            <span class="mini-cart-item-price">&#2547;{{ $wishlist->rel_to_product->after_discount }}</span>
                                                            <span class="mini-cart-item-quantity"><a href="{{ route('wishlist.remove',$wishlist->id) }}"><i
                                                                        class="ti-close"></i></a></span>
                                                        </div>
                                                    </div>
                                                @empty
                                                    <h5 class="text-center">Empty Wishlist</h5>
                                                @endforelse
                                                </div>
                                            ...
                                            <div class="mini-cart-content">
                                                ...
                                                <div class="mini-cart-items">
                                                    ...
                                                    @forelse ( App\Models\Cart::where('customer_id',Auth::guard('customer')->id())->get() as $cart )
                                                        <div class="mini-cart-item clearfix">
                                                            <div class="mini-cart-item-image">
                                                                <a href="{{ route('product.single',$cart->rel_to_product->slug) }}"><img src="{{ asset('uploads/product/preview') }}/{{ $cart->rel_to_product->preview }}" alt></a>
                                                            </div>
                                                            <div class="mini-cart-item-des">
                                                                @if (strlen($cart->rel_to_product->product_name)>12)
                                                                    <a href="{{ route('product.single',$cart->rel_to_product->slug) }}" title="{{ $cart->rel_to_product->product_name }}">{{ Str::substr($cart->rel_to_product->product_name,0,12).'..' }}</a>
                                                                @else
                                                                    <a href="{{ route('product.single',$cart->rel_to_product->slug) }}">{{ $cart->rel_to_product->product_name }}</a>
                                                                @endif
                        ...
                        <div class="row align-items-center">
                                                ...
                                                <li class="header-catagory-item">
                                                    ...
                                                    <ul class="header-catagory-single">
                                                        @foreach (App\Models\Subcategory::where('category_id',$category->id)->get() as $subcategory )  
                                                            <li><a href="{{ route('subcategories',$subcategory->id) }}">{{ $subcategory->subcategory_name }}</a></li>
                                                        @endforeach
                                                    </ul>
                                                </li>
                                            @endforeach
                            ...
                            <div class="col-lg-2 col-md-1 col-1">
                                <div class="header-right">
                                    <a href="{{ route('resent.view') }}" class="recent-btn"><i class="fi flaticon-refresh"></i> 
                    ...
                    <div class="row">
                        ...
                        <div class="col col-xl-3 col-lg-2 col-md-6 col-sm-12 col-12">
                            <div class="widget link-widget">
                                ...
                                <ul>
                                    @foreach (App\Models\SubProduct::all() as $sub_menu )
                                        <li><a href="{{ route('sub.product.name',$sub_menu->id) }}">{{ $sub_menu->sub_product }}</a></li>
                                    @endforeach
                                </ul>
                            </div>
                        </div>
                ...
                <div class="container">
                    <div class="row">
                        <div class="col col-xs-12">
                            <p class="copyright"> Copyright &copy; {{ carbon\carbon::now()->format('Y') }} Themart by <a href="index.html">wpOceans</a>.
                                All
                                Rights Reserved.</p>
                        </div>
                    </div>
    ...
    <script>
        ...
        $('.range_search').click(function(){
            ...
            var max = $('#max').val();
            var short = $('.sort').val();
            var link = "{{ route('shop') }}"+"?search_input="+search_input+"&category_id="+category_id+"&min="+min+"&max="+max+"&color_id="+color_id+"&size_id="+size_id+"&short="+short;
            window.location.href = link;
        });
